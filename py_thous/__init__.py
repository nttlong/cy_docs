...
def web_handler(path:str,method:WebMethods=WebMethods.GET):
    def warpper(obj):
        import inspect
        if inspect.isclass(obj):
            return __wrapper_class__(method.value,obj,path)
        elif callable(obj):
            return __wrapper_func__(method.value,obj, path)
    return warpper